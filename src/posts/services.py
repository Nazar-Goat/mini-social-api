...
class PostService:
    ...
    async def create_post(self, post_data: PostCreate, current_user: dict) -> PostOut:
        created_post = self.post_repository.create_post(post_data, current_user['id'])
    ...
    async def update_post(self, post_id: int, post_data: PostUpdate, current_user: dict) -> PostOut:
        post = await self.post_repository.get_post_by_id(post_id)
        if not post:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
